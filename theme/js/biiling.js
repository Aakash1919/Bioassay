function qvalidate()
{
    ...
    { 
        error_msg=1;
        error_msg30="Enter card holder's name";
        
        document.getElementById('cname').style.color="#f7290e";
        document.getElementById('cname').value=error_msg30;
    }
// inserted by WC 180924
   if(document.regform.payment_type.value=="")
    {
		
        error_msg=1;
        error_msg10="Select payment type";
        
    document.getElementById('pt').innerHTML=error_msg10;
    }

	 if(document.regform.po_num.value=="" || document.regform.battn.value=="Enter PO number")
	{
		//alert(document.regform.battn.value);
	 error_msg=1;
	 error_msg1="Enter PO number";
	
	 document.getElementById('po_num').style.color="#f7290e";
	 document.getElementById('po_num').value=error_msg1;

	}	
// End WC 180924 insert	
   if(document.regform.cardtype.value==0)
    {
		
        error_msg=1;
        error_msg31="Select card type";
        
    document.getElementById('ct').innerHTML=error_msg10;
    }
	
	  if(document.regform.month.value==0)
    {
		
		
        error_msg=1;
        error_msg31="Select month";
        
    document.getElementById('exm').innerHTML=error_msg10;
    }
	

	if(document.regform.csc.value=="")
    {
        error_msg=1;
        error_msg31="Enter CVV Code";
        
        document.getElementById('vcsc').style.color="#f7290e";
        document.getElementById('vcsc').value=error_msg31;
    }
	  if(document.regform.cardnumber.value=="")
    {
        error_msg=1;
        error_msg31="Enter card number";
        
        document.getElementById('cardnumber').style.color="#f7290e";
        document.getElementById('cardnumber').value=error_msg31;
    }
    ...
    if(document.regform.csc.value=="")
    {
        ...
        error_msg=1;
        error_msg=1;
        error_msg39="Enter CSC";
        document.getElementById('vcsc').style.borderColor="#f7290e";
        document.getElementById('vcsc').style.color="#f7290e";
        document.getElementById('vcsc').value=error_msg39;
    }
	...
	// Billing info
	 ...
	 if(document.regform.battn.value=="" || document.regform.battn.value=="Enter Contact person name")
	{
		//alert(document.regform.battn.value);
	 error_msg=1;
	 ...
	 if(document.regform.bcompany.value=="" || document.regform.bcompany.value=="Enter billing company name" )
	{
	 error_msg=1;
...
function  cardno()
{
  if(document.getElementById('cno').value=="This Is Not A Valid Credit Card Number")
	{
		document.getElementById('cno').value="";
	
		document.getElementById('cno').style.color="#000";
...
function  cwcode()
{
  if(document.getElementById('vcsc').value=="Enter CSC")
	{
		document.getElementById('vcsc').value="";
	
		document.getElementById('vcsc').style.color="#000";
...
function  cardname()
{
  ...
  if(document.getElementById('cname').value=="Enter card holder's name")
	{
		document.getElementById('cname').value="";
	
		document.getElementById('cname').style.color="#000";
...
function clearcno()
{
  if(document.getElementById('cardnumber').value=="Enter card number")
	{
		document.getElementById('cardnumber').value="";
		
		document.getElementById('cardnumber').style.color="#000";
...
function clearcsc()
{
  if(document.getElementById('vcsc').value=="Enter CVV Code")
	{
		document.getElementById('vcsc').value="";
...
function clrfx()
{
  if(document.getElementById('fedex_accnt').value=="Enter valid Fedex number")
	{
...
function  clrpo()
{
  if(document.getElementById('po_num').value=="Enter PO number")
	{
...
function expired( month, year ) {
    ...
    var expiresIn = new Date(year,month,0,0,0,0,0);		// create an expired on date object with valid thru expiration date
    //expiresIn.setMonth(expiresIn.getMonth()+1);		// adjust the month, to first day, hour, minute & second of expired month
    if( now.getTime() < expiresIn.getTime() ) return false;
    return true;									// then we get the miliseconds, and do a long integer comparison
}

        function fd_check(quantity)

		{
		  quantity=quantity.replace(/[^0-9]/g, '');
		  document.regform.fedex_accnt.value=quantity;
          document.regform.fedex_accnt.focus();
		}