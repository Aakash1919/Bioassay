...
class Checkout extends Public_Controller
{
	...
	public function billingQuotationProcess()
	{
		...
		if ($this->input->post() || $_SESSION['PreviousInfo']) {
			...
			if ($this->input->post('payment_type') == "Credit Card") {
				...
				$this->data['authToken'] = $this->generateAuthorizeToken($total, $this->cart->contents(), $extraInfo);
			}
	...
	*/
	...
	{
		$discountPost = $this->session->userdata('PromotionCode');
		$discountPost = isset($discountPost) ? $discountPost : array();
		$discountCode = $this->input->post('discountcod');
		array_push($discountPost, $discountCode);
		$cart = $this->cart->contents();
		if (!empty($discountPost)) {
			...
			if (empty($response)) {
				...
				$this->session->set_userdata('PromotionCode', $discountPost);
				$personID = $this->session->userdata('person_id');
				// if (!empty($personID)) {
	...
	*/
	...
	{
		$discountData = $this->session->userdata('discount_data');
		$promotionCodes = $this->session->userdata('PromotionCode');
		foreach($discountData as $discountKey => $discountValue) {
			if($code==$discountValue['code']) {
				unset($discountData[$discountKey]);
			}
		}
		foreach($promotionCodes as $promotionKey => $promotionValue) {
			if($code==$promotionValue) {
				unset($promotionCodes[$promotionKey]);
			}
		}
		$this->session->set_userdata('discount_data', $discountData);
		$this->session->set_userdata('PromotionCode', $promotionCodes);
		$personID = $this->session->userdata('person_id');
		if (!empty($personID)) {
		...
		if (isset($postData) && isset($sessionData) && $cart) {
			...
			if (!empty($orderID)) {
				...
				if ($this->input->post('payment_type') == "Credit Card") {
					...
					$response  = $this->generateAuthorizeToken($newTotal, $cart, $postData);
	...
	*/
	public function returnCountryCode($countryNamePost)
	{
		...
		$countryvalue = array(
			"US", "AL", "DZ", "AS", "AO", "AI", "AG", "AR", "AM", "AW", "AU", "AT", "AZ", "BS", "BH", "BD", "BB", "BY", "BE", "BZ", "BJ", "BM", "BT",
